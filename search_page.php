...
if(isset($_POST['add_to_cart'])){
   ...
   $check_cart_numbers = mysqli_query($conn, "SELECT * FROM `tbl_orders` WHERE prd_name = '$product_name' AND 	customer_id = '$user_id'") or die('query failed');

   if(mysqli_num_rows($check_cart_numbers) > 0){
      $message[] = 'already added to cart!';
   }else{
      mysqli_query($conn, "INSERT INTO `tbl_orders`(customer_id, staff_id, prd_name, prd_price, prd_quantity, prd_image) VALUES('$user_id', '2', '$product_name', '$product_price', '$product_quantity', '$product_image')") or die('query failed');
